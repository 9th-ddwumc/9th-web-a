...
function LpDetailModal({ isOpen, onClose, lpid }: LpDetailModalProps) {
  ...
  const unlikeMutation = useUnlikeLp(lpid!);
  ...
  const handleLike = () => {
    checkAuthAndExecute(() => {
      if (!lp || !user) return; // 데이터나 유저 정보가 없으면 실행 중지

      // 현재 좋아요 상태 확인
      const isLiked = lp.likes.some((like) => like.userId === user.id);
      
      if (isLiked) {
        // 이미 좋아요를 눌렀다면 -> 좋아요 취소 실행
        unlikeMutation.mutate();
      } else {
        // 좋아요를 누르지 않았다면 -> 좋아요 실행
        likeMutation.mutate();
      }
    });
  };
  ...
  //  좋아요 상태 및 카운트
  const isLikedByMe = lp?.likes.some((like) => like.userId === user?.id) || false;
  const likeCount = lp?.likes.length || 0;
  
  // 좋아요 버튼 비활성화 (요청 진행 중일 때)
        ...
        {isLpPending ? (
          ...
          <>
            ...
            >
              ...
              <button
                onClick={handleLike}
                disabled={isLikePending} // 요청 중 비활성화
                style={{
                  padding: '8px 16px',
                  // 좋아요 상태에 따라 배경색 변경
                  backgroundColor: isLikedByMe ? '#FF4B8C' : '#555',
                  border: 'none',
                  color: 'white',
                  borderRadius: '20px',
                  fontSize: '16px',
