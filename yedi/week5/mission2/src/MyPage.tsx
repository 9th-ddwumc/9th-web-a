...
function MyPage() {
  ...
  const handleGetUserInfo = async () => {
    ...
    try {
      ...
      setUserInfo(`API 응답: "${response.data}" (갱신된 토큰: ${JSON.parse(localStorage.getItem('accessToken') || 'null')})`);
    } catch (error: any) {
  ...
  return (
    <div style={{ padding: '20px', color: 'white', background: '#111', minHeight: '100vh' }}>
      ...
      {userInfo && (
        <pre style={{ marginTop: '20px', background: '#333', padding: '10px', borderRadius: '5px', color: 'white' }}>
          {userInfo}
        </pre>
      )}
    </div>
