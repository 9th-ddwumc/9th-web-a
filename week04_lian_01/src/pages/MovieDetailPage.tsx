...
type DetailBundle = { movie: MovieDetail; credits: CreditsResponse };

export default function MovieDetailPage() {
  const { movieId } = useParams<{ movieId: string }>();

  const { data, isLoading, isError } = useCustomFetch<DetailBundle>(
    async () => {
      if (!movieId) throw new Error("no id");
      const [m, c] = await Promise.all([
        tmdb.get<MovieDetail>(`/movie/${movieId}?language=ko-KR`).then(r => r.data),
        tmdb.get<CreditsResponse>(`/movie/${movieId}/credits?language=ko-KR`).then(r => r.data),
      ]);
      return { movie: m, credits: c };
    },
    [movieId]
  );
  ...
  if (isError || !data) {
    return <div className="mt-10 text-center text-red-500">영화 정보를 불러오지 못했어요.</div>;
  }

  const { movie, credits } = data;

  const tmdbImg = (p: string | null, size: "original" | "w500" | "w780" | "w185" = "original") =>
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div
        ...
        style={{
          backgroundImage: movie.backdrop_path ? `url(${tmdbImg(movie.backdrop_path, "original")})` : undefined,
        ...
        <div className="relative z-10 flex h-full items-center">
          <div className="mx-auto w-full max-w-5xl px-6">
            <div className="max-w-[640px] text-white drop-shadow-[0_1px_1px_rgba(0,0,0,0.35)]">
              ...
              {movie.tagline && <p className="mt-3 text-base italic text-gray-200">{movie.tagline}</p>}
              {movie.overview && <p className="mt-3 max-h-40 overflow-hidden text-sm leading-relaxed">{movie.overview}</p>}
